  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-6 md:p-12 z-10 text-white">
      ...
      <header className="pointer-events-auto">
        <h1 className="text-4xl md:text-6xl font-serif text-transparent bg-clip-text bg-gradient-to-r from-luxury-gold via-luxury-goldLight to-luxury-gold drop-shadow-lg">
          Signature Christmas
        </h1>
        <p className="text-luxury-silver mt-2 text-sm md:text-base tracking-widest uppercase opacity-80">
      ...
      <div className={`pointer-events-auto transition-opacity duration-700 ${isTree ? 'opacity-50 hover:opacity-100' : 'opacity-100'}`}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            {Object.values(THEMES).map((theme) => (
              <button
                ...
                className={`group relative w-12 h-12 rounded-full border-2 transition-all duration-300 overflow-hidden
                  ${currentTheme.id === theme.id ? 'border-luxury-gold scale-110 shadow-[0_0_15px_rgba(212,175,55,0.5)]' : 'border-white/20 hover:border-white/50'}
                `}
                title={theme.name}
              >
                <div 
          ...
          <div className="text-lg serif italic text-luxury-silver">
            {currentTheme.name}
          </div>
        </div>
      </div>
      ...
      <div className="pointer-events-auto flex justify-end items-end">
        <button
          ...
          className={`
            relative group overflow-hidden px-8 py-4 rounded-none border border-luxury-gold/50 
        ...
        >
          <div className="flex items-center gap-3 relative z-10">
            {isTree ? <RefreshCw className="w-5 h-5 text-luxury-gold" /> : <Wand2 className="w-5 h-5 text-luxury-gold" />}
            <span className="text-lg tracking-widest uppercase text-luxury-goldLight font-bold">
